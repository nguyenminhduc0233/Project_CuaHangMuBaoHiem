...
@WebServlet(name = "add-service", value = "/add-service")
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if(CustomerService.checkContainService(service)){
                request.setAttribute("notification",CustomerService.addPermissionManager(service));
                request.setAttribute("color","#FF0000");
            }else{
                String notification = CustomerService.addPermissionManager(service);
